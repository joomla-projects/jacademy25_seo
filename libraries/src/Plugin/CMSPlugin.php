...
abstract class CMSPlugin implements DispatcherAwareInterface, PluginInterface, LanguageAwareInterface
{
    ...
    public function loadLanguage($extension = '', $basePath = JPATH_ADMINISTRATOR)
    {
        ...
        $lang      = $this->getApplication() instanceof CMSApplicationInterface ? $this->getApplication()->getLanguage() : Factory::getLanguage();

        // If language already loaded, don't load it again.
        if ($lang->getPaths($extension)) {
            return true;
        }
        if ($lang->load($extension, $basePath)) {
            return true;
        }
        return $lang->load($extension, JPATH_PLUGINS . '/' . $this->_type . '/' . $this->_name);
    }
