...
class ApplicationModel extends FormModel implements MailerFactoryAwareInterface
{
    ...
    public function save($data)
    {
        ...
        if (\in_array($options['driver'], ['pgsql', 'postgresql']) && !preg_match('#^[a-zA-Z_][0-9a-zA-Z_$]*$#', (string) $options['database'])) {
            $app->enqueueMessage(Text::_('COM_CONFIG_FIELD_DATABASE_NAME_INVALID_MSG_POSTGRES'), 'warning');

            return false;
        }

        if (\in_array($options['driver'], ['mysql', 'mysqli']) && preg_match('#[\\\\\/]#', (string) $options['database'])) {
